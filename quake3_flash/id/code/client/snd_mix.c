...
#ifdef FLASH
		static float sound_samples[2 * PAINTBUFFER_SIZE];
		float f; float p = 0.004f / 32768.0f; int n = end - s_paintedtime; sum += n;
		
		if ( sum <= 8192 )
		{
			for( i = 0 ; i < n ; i++ )
			{
				f = (float)paintbuffer[i].left * p;
				f = __FloatSwap__(f);
				sound_samples[2*i+0] = f;
				f = (float)paintbuffer[i].right * p;
				f = __FloatSwap__(f);
				sound_samples[2*i+1] = f;
			}
			AS3_ByteArray_writeBytes( _flash_sample_data, sound_samples, 2 * n * sizeof(float) );
		}
#else		
